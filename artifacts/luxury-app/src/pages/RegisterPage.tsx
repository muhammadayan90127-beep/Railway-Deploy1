...
export const RegisterPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: `Welcome to MH Interior Design, ${data.user.name.split(' ')[0]}!` });
      setLocation("/");
    } catch {
    ...
    <div className="min-h-screen flex items-center justify-center bg-background relative overflow-hidden px-4 py-12">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md z-10">
        <div className="text-center mb-10">
          ...
          <p className="font-sans text-foreground/60 text-sm">Join us to book and track your consultations</p>
        </div>
        ...
        <div className="bg-card/60 backdrop-blur-xl border border-border/50 p-8 shadow-2xl relative">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              ...
              <Input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your full name" className="bg-background border-border/50 rounded-none focus-visible:ring-primary h-12" autoComplete="name" />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              <label className="font-sans text-xs uppercase tracking-wider text-foreground/70">Phone Number <span className="text-foreground/40">(Optional)</span></label>
              <Input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+91 98765 43210" className="bg-background border-border/50 rounded-none focus-visible:ring-primary h-12" autoComplete="tel" />
            </div>
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input
                  ...
                  placeholder="At least 6 characters" className="bg-background border-border/50 rounded-none focus-visible:ring-primary h-12 pr-12"
